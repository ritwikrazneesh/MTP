...
class PromptLearner(nn.Module):
    ...
    def compose_embeds(self) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        prefix = self.token_emb_fixed[:, : self.ctx_pos, :]
        suffix = self.token_emb_fixed[:, self.ctx_pos :, :]

        if self.n_ctx > 0:
            ctx_exp = self.ctx.unsqueeze(0).expand(N, -1, -1)
            embeds = torch.cat([prefix, ctx_exp, suffix], dim=1)   # depends on ctx
